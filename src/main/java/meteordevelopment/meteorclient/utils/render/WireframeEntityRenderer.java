...
public class WireframeEntityRenderer
{
    ...
    public static void render(Render3DEvent event, Entity entity, double scale, Color sideColor,
                              Color lineColor, ShapeMode shapeMode)
    {
        ...
        if (entityRenderer instanceof LivingEntityRenderer renderer)
        {
            ...
            if (entityRenderer instanceof PlayerEntityRenderer r)
            {
                ...
                BipedEntityModel.ArmPose armPose = PlayerEntityRenderer
                    .getArmPose((AbstractClientPlayerEntity) entity, Hand.MAIN_HAND);
                BipedEntityModel.ArmPose armPose2 = PlayerEntityRenderer
                    .getArmPose((AbstractClientPlayerEntity) entity, Hand.OFF_HAND);

                if (armPose.isTwoHanded())
                    armPose2 = livingEntity.getOffHandStack().isEmpty()
            ...
            float bodyYaw = MathHelper.lerpAngleDegrees(event.tickDelta, livingEntity.prevBodyYaw,
                livingEntity.bodyYaw);
            float headYaw = MathHelper.lerpAngleDegrees(event.tickDelta, livingEntity.prevHeadYaw,
                livingEntity.headYaw);
            float yaw = headYaw - bodyYaw;

            float animationProgress;
            if (livingEntity.hasVehicle()
                && livingEntity.getVehicle() instanceof LivingEntity livingEntity2)
            {
                bodyYaw = MathHelper.lerpAngleDegrees(event.tickDelta, livingEntity2.prevBodyYaw,
            ...
            float pitch = MathHelper.lerp(event.tickDelta, livingEntity.prevPitch,
            ...
            renderer.setupTransforms(livingEntity, matrices, animationProgress, bodyYaw,
                event.tickDelta, livingEntity.getScale());
            ...
            if (model instanceof AnimalModel m)
            {
                if (m.child)
                {
                    ...
                    matrices.translate(0.0D, m.childHeadYOffset / 16.0F,
                        m.childHeadZOffset / 16.0F);
                    if (model instanceof BipedEntityModel mo)
                        render(event.renderer, mo.head);
                    else
                        m.getHeadParts().forEach(
                            modelPart -> render(event.renderer, (ModelPart) modelPart));
                    matrices.pop();
                    matrices.push();
            ...
            } else
            {
                ...
                {
                    m.getParts()
                        .forEach(modelPart -> render(event.renderer, (ModelPart) modelPart));
        ...
        if (entityRenderer instanceof EndCrystalEntityRenderer renderer)
        {
            ...
            matrices.push();
            if (chamsEnabled)
                matrices.scale(2.0F * chams.crystalsScale.get().floatValue(),
                    2.0F * chams.crystalsScale.get().floatValue(),
                    2.0F * chams.crystalsScale.get().floatValue());
            else
            ...
            if (chamsEnabled)
                matrices.multiply(RotationAxis.POSITIVE_Y
                    .rotationDegrees(j * chams.crystalsRotationSpeed.get().floatValue()));
            else
                matrices.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(j));
            matrices.translate(0.0D, 1.5F + h / 2.0F, 0.0D);
            matrices.multiply(
                new Quaternionf().setAngleAxis(60.0F, EndCrystalEntityRenderer.SINE_45_DEGREES,
                    0.0F, EndCrystalEntityRenderer.SINE_45_DEGREES));
        ...
        } else if (entityRenderer instanceof BoatEntityRenderer renderer)
        {
            ...
            matrices.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(
                180.0F - MathHelper.lerp(event.tickDelta, entity.prevYaw, entity.getYaw())));
            ...
            if (h > 0.0F)
            {
                matrices.multiply(RotationAxis.POSITIVE_X.rotationDegrees(MathHelper.sin(h) * h * j
                    / 10.0F * (float) boatEntity.getDamageWobbleSide()));
            }
            ...
            if (!MathHelper.approximatelyEquals(k, 0.0F))
            {
                matrices.multiply(new Quaternionf().setAngleAxis(
                    boatEntity.interpolateBubbleWobble(event.tickDelta), 1.0F, 0.0F, 1.0F));
            }

            CompositeEntityModel<BoatEntity> boatEntityModel =
                renderer.texturesAndModels.get(boatEntity.getVariant()).getSecond();
            ...
            if (!boatEntity.isSubmergedInWater()
                && boatEntityModel instanceof ModelWithWaterPatch modelWithWaterPatch)
        ...
        } else if (entityRenderer instanceof ItemEntityRenderer)
        {
            ...
            event.renderer.box(dx + box.minX, dy + box.minY, dz + box.minZ, dx + box.maxX,
                dy + box.maxY, dz + box.maxZ, sideColor, lineColor, shapeMode, 0);
        }
    ...
    private static void render(Renderer3D renderer, ModelPart.Cuboid cuboid, double offsetX,
                               double offsetY, double offsetZ)
    {
        ...
        {
            // Transform positions
            pos1.set(quad.vertices[0].pos.x / 16, quad.vertices[0].pos.y / 16,
                quad.vertices[0].pos.z / 16, 1);
            pos1.mul(matrix);

            pos2.set(quad.vertices[1].pos.x / 16, quad.vertices[1].pos.y / 16,
                quad.vertices[1].pos.z / 16, 1);
            pos2.mul(matrix);

            pos3.set(quad.vertices[2].pos.x / 16, quad.vertices[2].pos.y / 16,
                quad.vertices[2].pos.z / 16, 1);
            pos3.mul(matrix);

            pos4.set(quad.vertices[3].pos.x / 16, quad.vertices[3].pos.y / 16,
                quad.vertices[3].pos.z / 16, 1);
            pos4.mul(matrix);
            ...
            if (shapeMode.sides())
            {
                renderer.triangles.quad(
                    renderer.triangles
                        .vec3(offsetX + pos1.x, offsetY + pos1.y, offsetZ + pos1.z)
                        .color(sideColor).next(),
                    renderer.triangles
                        .vec3(offsetX + pos2.x, offsetY + pos2.y, offsetZ + pos2.z)
                        .color(sideColor).next(),
                    renderer.triangles
                        .vec3(offsetX + pos3.x, offsetY + pos3.y, offsetZ + pos3.z)
                        .color(sideColor).next(),
                    renderer.triangles
                        .vec3(offsetX + pos4.x, offsetY + pos4.y, offsetZ + pos4.z)
                        .color(sideColor).next());
            }

            if (shapeMode.lines())
            {
                renderer.line(offsetX + pos1.x, offsetY + pos1.y, offsetZ + pos1.z,
                    offsetX + pos2.x, offsetY + pos2.y, offsetZ + pos2.z, lineColor);
                renderer.line(offsetX + pos2.x, offsetY + pos2.y, offsetZ + pos2.z,
                    offsetX + pos3.x, offsetY + pos3.y, offsetZ + pos3.z, lineColor);
                renderer.line(offsetX + pos3.x, offsetY + pos3.y, offsetZ + pos3.z,
                    offsetX + pos4.x, offsetY + pos4.y, offsetZ + pos4.z, lineColor);
                renderer.line(offsetX + pos1.x, offsetY + pos1.y, offsetZ + pos1.z,
                    offsetX + pos1.x, offsetY + pos1.y, offsetZ + pos1.z, lineColor);
            }
        }
    }


    public static List<RenderablePart> cloneEntityForRendering(Render3DEvent event, Entity entity,
                                                               Vec3d offset)
    {
        List<RenderablePart> parts = new ArrayList<>();
        ...
        if (entityRenderer instanceof LivingEntityRenderer renderer)
        {
            ...
            if (model instanceof AnimalModel m)
            {
                if (m.child)
                {
                    ...
                    matrices.translate(0.0D, m.childHeadYOffset / 16.0F,
                        m.childHeadZOffset / 16.0F);
                    if (model instanceof BipedEntityModel mo)
                        cloneRenderParts(parts, mo.head);
                    else
                        m.getHeadParts().forEach(
                            modelPart -> cloneRenderParts(parts, (ModelPart) modelPart));
                    matrices.pop();
                    matrices.push();
                    g = 1.0F / m.invertedChildBodyScale;
                    matrices.scale(g, g, g);
                    ...
                    if (model instanceof BipedEntityModel mo)
                    {
                        cloneRenderParts(parts, mo.body);
                        cloneRenderParts(parts, mo.leftArm);
                        cloneRenderParts(parts, mo.rightArm);
                        cloneRenderParts(parts, mo.leftLeg);
                        cloneRenderParts(parts, mo.rightLeg);
                    } else
                        m.getBodyParts().forEach(
                            modelPart -> cloneRenderParts(parts, (ModelPart) modelPart));
                    matrices.pop();
            ...
            } else
            {
                if (model instanceof SinglePartEntityModel m)
                {
                    cloneRenderParts(parts, m.getPart());
                } else if (model instanceof CompositeEntityModel m)
                {
                    m.getParts()
                        .forEach(modelPart -> cloneRenderParts(parts, (ModelPart) modelPart));
                } else if (model instanceof LlamaEntityModel m)
                {
                    if (m.child)
                    {
                        ...
                        matrices.scale(0.71428573F, 0.64935064F, 0.7936508F);
                        matrices.translate(0.0D, 1.3125D, 0.2199999988079071D);
                        cloneRenderParts(parts, m.head);
                        matrices.pop();
                        matrices.push();
                        matrices.scale(0.625F, 0.45454544F, 0.45454544F);
                        matrices.translate(0.0D, 2.0625D, 0.0D);
                        cloneRenderParts(parts, m.body);
                        matrices.pop();
                        matrices.push();
                        matrices.scale(0.45454544F, 0.41322312F, 0.45454544F);
                        matrices.translate(0.0D, 2.0625D, 0.0D);
                        cloneRenderParts(parts, m.rightHindLeg);
                        cloneRenderParts(parts, m.leftHindLeg);
                        cloneRenderParts(parts, m.rightFrontLeg);
                        cloneRenderParts(parts, m.leftFrontLeg);
                        cloneRenderParts(parts, m.rightChest);
                        cloneRenderParts(parts, m.leftChest);
                        matrices.pop();
                    } else
                    {
                        cloneRenderParts(parts, m.head);
                ...
                } else if (model instanceof RabbitEntityModel m)
                {
                    if (m.child)
                    {
                        ...
                        matrices.translate(0.0D, 1.375D, 0.125D);
                        cloneRenderParts(parts, m.head);
                        cloneRenderParts(parts, m.leftEar);
                        cloneRenderParts(parts, m.rightEar);
                        cloneRenderParts(parts, m.nose);
                        matrices.pop();
                        matrices.push();
                        matrices.scale(0.4F, 0.4F, 0.4F);
                        matrices.translate(0.0D, 2.25D, 0.0D);
                        cloneRenderParts(parts, m.leftHindLeg);
                        cloneRenderParts(parts, m.rightHindLeg);
                        cloneRenderParts(parts, m.leftHaunch);
                        cloneRenderParts(parts, m.rightHaunch);
                        cloneRenderParts(parts, m.body);
                        cloneRenderParts(parts, m.leftFrontLeg);
                        cloneRenderParts(parts, m.rightFrontLeg);
                        cloneRenderParts(parts, m.tail);
                        matrices.pop();
                    } else
                    {
                        matrices.push();
        ...
        if (entityRenderer instanceof EndCrystalEntityRenderer renderer)
        {
            ...
            matrices.translate(0.0D, -0.5D, 0.0D);
            if (crystalEntity.shouldShowBottom())
                cloneRenderParts(parts, renderer.bottom);
            ...
            matrices.multiply(
                new Quaternionf().setAngleAxis(60.0F, EndCrystalEntityRenderer.SINE_45_DEGREES,
                    0.0F, EndCrystalEntityRenderer.SINE_45_DEGREES));
            if (!chamsEnabled || chams.renderFrame1.get())
                cloneRenderParts(parts, renderer.frame);
            matrices.scale(0.875F, 0.875F, 0.875F);
            matrices.multiply(
                new Quaternionf().setAngleAxis(60.0F, EndCrystalEntityRenderer.SINE_45_DEGREES,
                    0.0F, EndCrystalEntityRenderer.SINE_45_DEGREES));
            ...
            if (chamsEnabled)
                ...
                matrices.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(j));
            if (!chamsEnabled || chams.renderCore.get())
                cloneRenderParts(parts, renderer.core);
            matrices.pop();
            matrices.pop();
        } else if (entityRenderer instanceof BoatEntityRenderer renderer)
        {
            ...
            matrices.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(90.0F));
            boatEntityModel.setAngles(boatEntity, event.tickDelta, 0.0F, -0.1F, 0.0F, 0.0F);
            boatEntityModel.getParts().forEach(modelPart -> cloneRenderParts(parts, modelPart));
            if (!boatEntity.isSubmergedInWater()
                && boatEntityModel instanceof ModelWithWaterPatch modelWithWaterPatch)
                cloneRenderParts(parts, modelWithWaterPatch.getWaterPatch());
    ...
    public static void render(Render3DEvent event, Vec3d offset, List<RenderablePart> parts,
