...
public class AutoCrystal extends Module {
    ...
    private final Setting<Boolean> pauseEatPlace = sgGeneral.add(new BoolSetting.Builder().name("pause-eat-place")
    ...
    private final Setting<Boolean> pauseEatBreak = sgGeneral.add(new BoolSetting.Builder().name("pause-eat-break")
    ...
    // -- Place -- //
    private final Setting<Double> placeSpeedLimit = sgPlace.add(new DoubleSetting.Builder().name("place-speed-limit")
    ...
    private final Setting<Boolean> ignoreItems = sgPlace.add(new BoolSetting.Builder()
            .name("ignore-items")
            .description(
                    "Places on items. Good for low ping.")
            .defaultValue(true).build());

    // -- Face Place -- //
    private final Setting<Boolean> facePlaceMissingArmor = sgFacePlace
            .add(new BoolSetting.Builder().name("face-place-missing-armor")
                    .description("Face places on missing armor").defaultValue(true).build());

    private final Setting<Keybind> forceFacePlaceKeybind = sgFacePlace
            .add(new KeybindSetting.Builder().name("force-face-place")
    ...
    // -- Break -- //
    private final Setting<Double> breakSpeedLimit = sgBreak.add(new DoubleSetting.Builder().name("break-speed-limit")
    ...
    private final Setting<Double> breakDelay = sgBreak.add(new DoubleSetting.Builder().name("break-delay")
            .description("The number of seconds to wait to retry breaking a crystal.")
            .defaultValue(0.05).min(0).sliderMax(0.6).build());
    // -- Rotate -- //
    private final Setting<Boolean> rotatePlace = sgRotate.add(new BoolSetting.Builder().name("rotate-place")
            .description("Rotates server-side towards the crystals when placed.")
            .defaultValue(false).build());

    private final Setting<Boolean> rotateBreak = sgRotate.add(new BoolSetting.Builder().name("rotate-break")
            .description("Rotates server-side towards the crystals when broken.")
            .defaultValue(true).build());

    // -- Swing -- //
    private final Setting<SwingMode> breakSwingMode = sgSwing
            .add(new EnumSetting.Builder<SwingMode>().name("break-swing-mode")
                    .description("Mode for swinging your hand when breaking")
                    .defaultValue(SwingMode.None).build());

    private final Setting<SwingMode> placeSwingMode = sgSwing
            .add(new EnumSetting.Builder<SwingMode>().name("place-swing-mode")
    ...
    public boolean breakCrystal(Entity entity) {
        ...
        brokenCrystals.add(entity.getId());
    ...
    public void preplaceCrystal(BlockPos crystalBlockPos, boolean snapAt) {
        if (pauseEatPlace.get() && mc.player.isUsingItem()) {
            return;
        }
    ...
    private void onEntity(EntityAddedEvent event) {
        ...
        BlockPos blockPos = BlockPos.ofFloored(entity.getPos()).down();
        if (crystalPlaceDelays.containsKey(blockPos)) {
            crystalPlaceDelays.remove(blockPos);
        }
    ...
    private boolean intersectsWithEntities(Box box) {
        return EntityUtils.intersectsWithEntity(box,
                entity -> {
                    ...
                    if (brokenCrystals.contains(entity.getId())) {
                        return false;
                    }

                    // age < 10 to avoid stupid placements on items
                    if (ignoreItems.get() && entity instanceof ItemEntity item && item.age < 10) {
                        return false;
                    }
